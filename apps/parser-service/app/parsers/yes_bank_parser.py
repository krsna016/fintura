...
class YesBankParser(BaseBankParser):
    def detect(self, first_page_text: str) -> bool:
        keywords = ["YES BANK", "YES Bank", "yesbank"]
    ...
    def parse(self, file_path: str) -> pd.DataFrame:
        if file_path.endswith('.pdf'):
            ...
            raise ValueError("Unsupported file format for YES Bank statement")
        ...
        for r in raw_rows:
            try:
                ...
                if len(r) >= 6:
                    ...
                    amount_cr = self.clean_float(credit_val)
                    amount_dr = self.clean_float(debit_val)
