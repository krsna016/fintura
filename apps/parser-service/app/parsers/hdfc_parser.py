...
class HDFCParser(BaseBankParser):
    def detect(self, first_page_text: str) -> bool:
        keywords = ["HDFC BANK", "HDFC Bank", "hdfcbank"]
    ...
    def parse(self, file_path: str) -> pd.DataFrame:
        # HDFC parser standard fallback (parsing using dynamic column detection)
        # Similar to ICICI/YES Bank table-based logic
        if file_path.endswith('.pdf'):
            # Simple fallback to dynamic dataframe parsing or text parsing
            import pdfplumber
            rows = []
            ...
            raise ValueError("Unsupported format for HDFC statement")
    ...
    def _normalize_rows(self, raw_rows: list[list[str]]) -> pd.DataFrame:
        ...
        for r in raw_rows:
            try:
                ...
                logger.error(f"Error parsing row: {e} - {r}")
                continue
